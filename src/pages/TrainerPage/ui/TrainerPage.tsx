...
const TrainerInner: React.FC<TrainerPageProps> = memo(
  ({ words, theme }): React.JSX.Element => {
    ...
    const wordOnFailLocked = useCallback((...args: any[]) => {
      if (clickLocked) return;
      setClickLocked(true);
      // оригинальный обработчик
      (wordOnFail as any)(...args);
    }, [clickLocked, wordOnFail]);

    const wordOnSuccessLocked = useCallback((...args: any[]) => {
      if (clickLocked) return;
      setClickLocked(true);
      (wordOnSuccess as any)(...args);
    }, [clickLocked, wordOnSuccess]);
    ...
    useEffect(() => {
      // как только выбранное слово обновилось — разблокируем клики
      if (randomWordId !== null) setClickLocked(false);
    }, [randomWordId]);
    ...
    }, [checkArrowsPress, words, clickLocked]);
            ...
            {!totalTime ? (
              <>
                ...
                {theme === 'Ударения' && (
                  <Flex width="100">
                    <Hint
                      text={`Если ответите правильно с первого, слово зачтётся сразу. Если допустите ошибку, нужно будет ответить три раза подряд правильно.
1 раз пройдите тренажер без выбора режима, потом выполните работу над ошибками. 
2 раз выполните в режиме "одна жизнь"`}
                      textClassName={styles.TrainerPage__hint}
                    />
                  </Flex>
                )}
...
2 раз выполните в режиме "одна жизнь"`}
                ...
                {randomWord && (
                  <Flex width="100" direction="column" gap="10" data-click-locked={clickLocked}>
                    {Object.entries(
                      generateTrainerWords(
                        randomWord,
                        randomWordsIsReverse,
                        wordOnFailLocked,      // ⟵ было wordOnFail
                        wordOnSuccessLocked,   // ⟵ было wordOnSuccess
                        words,
                        showNewWord,
                        theme,
                      ),
